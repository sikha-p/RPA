    ...
    for item in workitemsGroupByQueue:
        ...
        try:
            ...
            if response.status_code == 200:  # Adjust based on your API's success status code
                json_response = response.json()
                log(f"Updated workitems status as ON_HOLD {json.dumps(json_payload)}", "debug")
...
def fetch_unprocessed_workitems_from_feederqueues(Queues):
    ...
    for detail in Queues:
        ...
        [mapped_items.update({"ProcessPriority": detail["ProcessPriority"], "QueuePriority": detail["QueuePriority"]}) for mapped_items in mapped_items]
    ...
    filteredlist = filterHighPriorityWorkitems(all_results)
    #sorted_items = sort_work_items(all_results, PRIORITY_COLUMNS)
    log(f"fetch_unprocessed_workitems_from_feederqueues - sorted Items : {json.dumps(filteredlist)}", "debug")
    return filteredlist


def filterHighPriorityWorkitems(data):
    # Group queues by process name
    process_groups = {}
    for queue in data:
        process_name = queue['ProcessName']
        if process_name not in process_groups:
            process_groups[process_name] = []
        process_groups[process_name].append(queue)

    # Process each group
    for process_name, queues in process_groups.items():
        total_device_count = queues[0]['DeviceCount'] #sum(queue['DeviceCount'] for queue in queues)
        all_work_items = []

        # Collect all work items from the queues
        for queue in queues:
            for work_item in queue['WorkItems']:
                work_item['QueueID'] = queue['QueueID']  # Add QueueID to work item for reference
                all_work_items.append(work_item)

        # Sort work items by QueuePriority (highest priority first) and WorkItemPriority
        sorted_work_items = sorted(all_work_items, key=lambda x: int(x['QueuePriority']))

        # Select top work items based on total_device_count
        selected_items = sorted_work_items[:total_device_count]

        # Update queues with selected work items and remove non-selected ones
        for queue in queues:
            queue['WorkItems'] = [
                item for item in selected_items if item['QueueID'] == queue['QueueID']
            ]
...
def get_high_priority_workitems(required_count, queues):
    ...
    # Sort work items by ProcessPriority (ascending) and QueuePriority (ascending)
    all_workitems.sort(
        key=lambda x: (int(x['WorkItem']['ProcessPriority']), int(x['WorkItem']['QueuePriority']))
    )
...
def allocate_items_based_on_ratio(all_results):
    ...
    # Dictionary to track unique DeviceCounts by ProcessName
    unique_counts = {}
    ...
    for item in all_results:
        ...
        # Add the DeviceCount only if the ProcessName is not already counted
        if process_name not in unique_counts:
            unique_counts[process_name] = device_count

    # Calculate the total unique DeviceCount
    total_device_count = sum(unique_counts.values())
...
def check_queueEligibility(row):
...
def checkQueueEligibilityBasedOnLastPoll(filename):
    ...
    # Ensure 'LastPoll' is in date  time format

    df['LastPoll'] = pd.to_datetime(df['LastPoll'], format="%Y-%m-%d %H:%M:%S")
    # Apply the function to the DataFrame
    df['AddQueue'] = df.apply(check_queueEligibility, axis=1)
...
def updateLastCron(filename):
    ...
    # Process the input to populate FEEDER_QUEUE_DETAILS
    for process in PROCESS_DETAILS:
        ...
        for queue in process["FeederQueueDetails"]:
            ...
            FEEDER_QUEUE_DETAILS.append(queue_details)
    ...
    if not os.path.exists(filename):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as csvfile:
            ...
            writer.writerows(FEEDER_QUEUE_DETAILS)  # Write all rows
...
try:
    ...
    with open("config.json", "r") as file:
        config = json.load(file)
    logfile_folder_path = config['logfile_folder_path']
    log_file_path = logfile_folder_path + "\\WLM_MaxDeviceUtility_PrioritySync_Solution.log"
    initialize_logger(log_file_path=log_file_path, log_level=log_level)
    log('Logs entry started', 'debug')


    BASE_CR_URL = config['base_cr_url']
    PROCESS_DETAILS = config['ProcessDetails']
    ...
    sort_and_add_workitems_to_activequeue_from_feederqueues(filtered_feederQueues)
